...
const AILawyer: React.FC<AILawyerProps> = ({ laws, userSituation }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await chatWithLawyer(messages, userMessage, laws, userSituation);
  ...
  return (
    <>
      ...
      <button 
        ...
        className="fixed bottom-6 right-6 flex h-14 w-14 items-center justify-center rounded-full bg-accent-color text-white shadow-lg transition-transform hover:scale-110 active:scale-95 z-50"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="fixed bottom-24 right-6 z-50 flex h-[500px] w-[350px] flex-col overflow-hidden rounded-2xl bg-card-bg shadow-2xl border border-border-color md:w-[400px]"
          >
            {/* Header */}
            <div className="flex items-center justify-between bg-accent-color px-4 py-3 text-white">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-[10px] opacity-80 uppercase tracking-widest">Powered by Gemini</span>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg, idx) => {
                // Check if message mentions a law with a poll
                const mentionedLaw = laws.find(l => msg.text.includes(l.title) || (l.id && msg.text.includes(l.id)));
                ...
                return (
                  ...
                  >
                    <div className={`flex max-w-[85%] gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                      ...
                      <div className={`rounded-2xl px-3 py-2 text-sm leading-relaxed ${msg.role === 'user' ? 'bg-accent-color text-white rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-tl-none'}`}>
                        {msg.text}
                      </div>
                    </div>
                      ...
                      <div className="ml-10 mt-2 max-w-[80%] rounded-xl border border-border-color bg-white p-3 shadow-sm">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {mentionedLaw.poll.options.map(opt => (
                            <button 
                              key={opt.label}
                              className="rounded-full border border-accent-color/30 bg-accent-color/5 px-3 py-1 text-[10px] font-bold text-accent-color transition-colors hover:bg-accent-color/10"
                            >
                              {opt.label}
                            </button>
                          ))}
            ...
            <div className="border-t border-border-color p-3">
              <div className="flex items-center gap-2 rounded-full border border-border-color bg-gray-50 px-3 py-1.5 focus-within:ring-2 focus-within:ring-accent-color">
                ...
                <button 
                  ...
                  className="rounded-full bg-accent-color p-1.5 text-white disabled:opacity-50"
                >
                  <Send size={16} />
                </button>
              </div>
