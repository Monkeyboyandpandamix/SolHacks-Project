        ...
        <div className="space-y-8">
          {selectedRegion ? (
            ...
            >
              ...
              <div className="rounded-[32px] border-2 border-slate-100 bg-white p-6 shadow-xl shadow-slate-200/30">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Sentiment Heatmap</p>
                <div className="mt-4 space-y-3">
                  {laws
                    ...
                    .slice(0, 3)
                    .map(law => {
                      const sentiment = Math.max(0, (law.votes?.support || 0) - (law.votes?.oppose || 0));
                      return (
                        <div key={`sentiment-${law.id}`}>
                          <div className="mb-1 flex items-center justify-between gap-4">
                            <p className="text-xs font-black text-slate-900 line-clamp-1">{law.title}</p>
                            <span className="text-[10px] font-black uppercase tracking-widest text-indigo-600">{sentiment}</span>
                          </div>
                          <div className="h-2 rounded-full bg-slate-100">
                            <div className="h-2 rounded-full bg-indigo-600" style={{ width: `${Math.min(100, 20 + sentiment)}%` }} />
                          </div>
                        </div>
                      );
                    })}
